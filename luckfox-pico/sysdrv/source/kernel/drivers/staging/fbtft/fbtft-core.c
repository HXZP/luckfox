...
{
/*起始地址和结束地址*/
#define LCD_ADRR_XSTART 0x28
#define LCD_ADRR_XEND   0x117
#define LCD_ADRR_YSTART 0x35
#define LCD_ADRR_YEND   0xBB
				 ...
				 unsigned int end_line)
{
	...
	dev_info(par->info->device, "%s(start_line=%u, end_line=%u)\n",
		      __func__, start_line, end_line);
...
static int fbtft_init_display_from_property(struct fbtft_par *par)
{
	...
	while (index < count) {
		if (val & FBTFT_OF_INIT_CMD) {
			...
			/* make debug message */
			dev_info(dev, "init: write_register:\n");
//			fbtft_par_dbg(DEBUG_INIT_DISPLAY, par,
//				      "init: write_register:\n");
			for (j = 0; j < i; j++)
				dev_info(dev, "buf[%d] = %02X\n", j, buf[j]);
//				fbtft_par_dbg(DEBUG_INIT_DISPLAY, par,
//					      "buf[%d] = %02X\n", j, buf[j]);
...
int fbtft_init_display(struct fbtft_par *par)
{
	...
	while (i < FBTFT_MAX_INIT_SEQUENCE) {
		...
		case -1:
			...
			dev_info(par->info->device, "init: write(0x%02X) %s\n", par->init_sequence[i], msg);
